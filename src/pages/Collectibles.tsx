...
function gainLoss(c: Collectible) {
  return c.current_value - c.purchase_price;
}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-border py-14 text-center">
      ...
      <p className="text-sm text-muted-foreground">
        No collectibles yet. Add your first item to track your collection.
      </p>
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {existing ? "Edit Item" : "Add Collectible"}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <Field label="Name">
            <input
              ...
              value={f.name}
              onChange={set("name")}
              placeholder="1952 Topps Mickey Mantle #311"
            />
          </Field>
          ...
          <Field label="Notes">
            <textarea
              ...
              value={f.notes}
              onChange={set("notes")}
              placeholder="Graded PSA 7, provenance…"
            />
          </Field>
        </div>
            ...
            {rows.map((r) => (
              ...
              >
                <td className="px-5 py-3">
                  <Badge
                    variant="outline"
                    className={categoryBadge(r.category)}
                  >
                    {r.category}
                  </Badge>
                </td>
...
export default function Collectibles() {
  ...
  const bestPerformer = items.length
    ? items.reduce(
        (best, i) => (gainLossPct(i) > gainLossPct(best) ? i : best),
        items[0],
      )
    : null;
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <KpiCard
              ...
              sub={
                totalCost > 0 ? fmtPct((totalGL / totalCost) * 100) : undefined
              }
            />
            <KpiCard
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <Badge
                                variant="outline"
                                className={categoryBadge(item.category)}
                              >
                                {item.category}
                              </Badge>
                            </td>
                            <td className="px-4 py-3">
                              <Badge
                                variant="outline"
                                className={conditionBadge(item.condition)}
                              >
                                {item.condition}
                              </Badge>
                            </td>
