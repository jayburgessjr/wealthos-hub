...
export default function Compound() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["compound_settings"] }),
  });
  ...
  const reverse = useMemo(
    () =>
      reverseCalc({
        startingCapital: starting,
        monthlyContribution: monthly,
        targetCapital: goalTarget,
        timeHorizonMonths: goalMonths,
      }),
  ...
  const ttg = useMemo(
    () =>
      timeToGoal({
        startingCapital: starting,
        monthlyContribution: goalMonthlyAdd,
        targetCapital: goalTarget,
        riskTier,
      }),
      ...
      <SubscriptionGate>
        <div className="space-y-6">
          ...
          >
            <div>
              ...
              <p className="mt-1.5 text-sm sm:text-[13px] text-muted-foreground hidden sm:block">
                // Every dollar assigned. Every return reinvested. Wealth
                compounded systematically.
              </p>
            </div>
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="flex items-center justify-center gap-2 rounded-lg bg-primary px-4 sm:px-6 py-2.5 font-display text-[13px] font-bold text-primary-foreground transition-shadow hover:shadow-lg w-full sm:w-auto"
            >
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <div className="flex flex-col gap-2.5 p-5">
                ...
                <span className="font-mono text-lg font-semibold text-bullish">
                  {reinvestPct}%
                </span>
                <input
                  type="range"
            ...
            <div className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="flex flex-col gap-4 p-5">
                    ...
                    <div key={s.label} className="flex items-center gap-2.5">
                      ...
                      <div className="flex-1 h-1.5 rounded-full bg-border overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${mcMax > 0 ? (s.value / mcMax) * 100 : 0}%`,
                            transition: "width 0.8s ease",
                          }}
                        />
                      </div>
                      <span className="w-[72px] flex-shrink-0 text-right text-sm font-medium text-foreground">
                        {fmtFull(s.value)}
                      </span>
                    </div>
                  ))}
                ...
                <div className="flex justify-between border-t border-border pt-3">
                  <ProbItem
                    value={`${mc.probability_double.toFixed(0)}%`}
                    label="Prob. 2x"
                    green
                  />
                  <ProbItem
                    value={`${(100 - mc.probability_of_loss).toFixed(0)}%`}
                    label="Prob. Profit"
                    green
                  />
                  <ProbItem
                    value={`${mc.probability_of_loss.toFixed(0)}%`}
                    label="Prob. Loss"
                  />
                </div>
          ...
          >
            <div className="border-b border-border px-5 py-3.5">
              <span className="font-display text-[11px] font-bold uppercase tracking-[1.2px] text-muted-foreground">
                Strategy Unlock Timeline
              </span>
              <p className="mt-0.5 text-xs text-muted-foreground">
                Capital milestones that unlock new wealth strategies
              </p>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`rounded-lg border overflow-hidden transition-all ${
                      isCurrent
                        ? "border-bullish shadow-[0_0_20px_hsl(160_100%_45%/0.1)] opacity-100"
                        : unlocked
                  ...
                  >
                    <div className="flex items-center justify-between border-b border-border px-4 py-3">
                      ...
                      <span className="text-xs text-muted-foreground">
                        ${tier.min_capital.toLocaleString()}
                        {tier.max_capital < Infinity
                          ? `–$${tier.max_capital.toLocaleString()}`
                          : "+"}
                      </span>
                      {!unlocked && (
                        <Lock className="ml-1 h-3 w-3 text-muted-foreground opacity-50" />
                      )}
                    </div>
                      ...
                      {tier.strategies.map((s) => (
                        ...
                        >
                          ...
                          <span className="text-sm text-muted-foreground">
                            est. {(s.avg_monthly_return * 100).toFixed(1)}%/mo
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="border-b md:border-b-0 md:border-r border-border p-4 sm:p-6">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <GoalInput
                    label="Monthly Add ($)"
                    value={goalMonthlyAdd}
                    onChange={setGoalMonthlyAdd}
                  />
                </div>
                <div className="mt-4 rounded-lg border border-border bg-surface p-4">
                  <div className="font-display text-[26px] font-extrabold text-bullish">
                    {ttg.label}
                  </div>
              ...
              <div className="p-4 sm:p-6">
                ...
                <div className="mt-4 rounded-lg border border-border bg-surface p-4">
                  <div className="font-display text-[26px] font-extrabold text-bullish">
                    {reverse.required_monthly_pct}%/mo
                  </div>
                  <div className="mt-1 text-xs text-muted-foreground">
                    required monthly return
                  </div>
                  <p
                    className={`mt-2.5 text-xs leading-relaxed ${reverse.is_achievable ? "text-muted-foreground" : "text-bearish"}`}
                  >
                    {reverse.verdict}
                  </p>
                </div>
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="sticky top-0 bg-surface px-4 py-3 text-left text-xs font-normal uppercase tracking-[1px] text-muted-foreground whitespace-nowrap border-b border-border"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {projection.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 font-mono text-xs text-bullish">
                        {fmtFull(r.monthly_return_dollars)}
                      </td>
                      <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">
                        {r.monthly_return_pct}%
                      </td>
                      <td className="px-4 py-2.5 font-mono text-xs text-bullish">
                        {fmtFull(r.cumulative_return)}
                      </td>
                      <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">
                      ...
                      <td className="px-4 py-2.5">
                        <span
                          ...
                          style={{
                            backgroundColor: r.tier_color + "20",
                            color: r.tier_color,
                          }}
                        >
                          {r.tier_label}
                      ...
                      <td className="px-4 py-2.5 font-mono text-xs text-neutral">
                        {r.strategies_available}
                      </td>
                      <td className="px-4 py-2.5 text-sm text-muted-foreground">
                        {r.next_unlock}
                      </td>
  ...
  return (
    <div className="flex flex-col gap-2.5 border-b sm:border-b-0 sm:border-r border-border p-4 sm:p-5 last:border-b-0 last:border-r-0">
      ...
      <div className="relative flex items-center">
        ...
        />
        {suffix && (
          <span className="absolute right-3 font-mono text-xs text-muted-foreground">
            {suffix}
          </span>
        )}
      </div>
...
function SummaryStat({
  ...
  return (
    <div
      className={`relative overflow-hidden p-4 sm:p-5 ${!last ? "border-b sm:border-b lg:border-b-0 border-r-0 sm:border-r lg:border-r border-border" : ""}`}
    >
      <div
        className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${accentColors[accent] || ""} to-transparent`}
      />
      <div className="text-xs uppercase tracking-[1px] text-muted-foreground mb-1.5">
        {label}
      </div>
      <div className="font-display text-lg sm:text-[22px] font-extrabold leading-none mb-1">
        {value}
      </div>
...
function ProbItem({
  ...
  return (
    <div className="text-center">
      <div
        className={`font-display text-xl font-extrabold ${green ? "text-bullish" : "text-bearish"}`}
      >
        {value}
      </div>
...
function GoalInput({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3">
      <span className="sm:w-36 flex-shrink-0 text-xs text-muted-foreground">
        {label}
      </span>
      <input
        type="number"
