...
import {
  ...
  type AssetInput,
  type QuantumResult,
} from "@/lib/quantumOptimizer";

// ── Default asset universe — symbols only, user must enter current prices ──
const DEFAULT_ASSETS: (Omit<AssetInput, "price" | "changePct"> & {
  price: string;
  changePct: string;
})[] = [
  { symbol: "SPY", price: "", changePct: "", assetClass: "equity" },
  { symbol: "QQQ", price: "", changePct: "", assetClass: "equity" },
  { symbol: "BTC", price: "", changePct: "", assetClass: "crypto" },
  { symbol: "ETH", price: "", changePct: "", assetClass: "crypto" },
  { symbol: "GLD", price: "", changePct: "", assetClass: "commodity" },
  { symbol: "TLT", price: "", changePct: "", assetClass: "bond" },
  { symbol: "NVDA", price: "", changePct: "", assetClass: "equity" },
  { symbol: "AAPL", price: "", changePct: "", assetClass: "equity" },
];
...
function MetricCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      ...
      <p
        className={`font-mono text-xl font-black mt-1 ${color ?? "text-foreground"}`}
      >
        {value}
      </p>
...
export default function Quantum() {
  ...
  const addAsset = () => {
    setAssets((prev) => [
      ...prev,
      { symbol: "", price: "100", changePct: "0", assetClass: "equity" },
    ]);
  };
  ...
  const run = useCallback(async () => {
    ...
    appendLog(
      `[QE] Capital: $${cap.toLocaleString()}  Risk: ${riskTolerance}/10`,
    );
    await new Promise((r) => setTimeout(r, 150));
    appendLog(
      `[MC] Seeding ${mcRuns} Monte Carlo paths (Box-Muller Gaussian)…`,
    );
    ...
    const res = await new Promise<QuantumResult>((resolve) => {
      setTimeout(() => {
        resolve(
          optimizePortfolio(validAssets, cap, riskTolerance, mcRuns, saIter),
        );
      }, 50);
    });
  ...
  // Donut slices
  const donutSlices =
    result?.allocation.slice(0, 8).map((a) => ({
      pct: a.percentage,
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Monte Carlo simulation · Simulated Annealing · VaR / CVaR ·
                Sharpe optimisation
              </p>
            </div>
              ...
              <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`font-mono text-xs font-black ${riskColor(riskTolerance)}`}
                    >
                        ...
                        {assets.map((asset, i) => (
                          <div key={i} className="flex items-center gap-2">
                            ...
                            <input
                              value={asset.price}
                              onChange={(e) =>
                                updateAsset(i, "price", e.target.value)
                              }
                              placeholder="Price"
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateAsset(i, "changePct", e.target.value)
                              }
                              placeholder="%chg"
                              className="w-16 rounded-lg border border-border bg-background px-2 py-1.5 font-mono text-xs text-foreground outline-none"
                            />
                            <select
                              ...
                              onChange={(e) =>
                                updateAsset(i, "assetClass", e.target.value)
                              }
                ...
                <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-border py-24 text-center">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Configure parameters and press Run to optimise your
                      portfolio
                    </p>
                  </div>
                  <button
                    onClick={run}
                    className="flex items-center gap-2 rounded-xl border border-primary/30 bg-primary/10 px-5 py-2.5 font-mono text-xs font-bold text-primary hover:brightness-110"
                  >
                  ...
                  >
                      ...
                      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div className="space-y-1.5">
                          {result.allocation.map((slice, i) => {
                            const color =
                              ASSET_CLASS_COLORS[slice.assetClass] ?? "#94a3b8";
                            return (
                              <motion.div
                                key={slice.symbol}
                              ...
                              >
                                ...
                                />
                                <span className="font-mono text-xs font-bold text-foreground w-14 shrink-0">
                                  {slice.symbol}
                                </span>
                                ...
                                <span
                                  className="font-mono text-xs font-black w-12 text-right"
                                  style={{ color }}
                                >
                                ...
                                <span className="text-xs text-muted-foreground w-20 text-right">
                                  $
                                  {slice.dollarAmount.toLocaleString(
                                    undefined,
                        ...
                        <tbody>
                          {result.allocation.map((slice, i) => {
                            ...
                            return (
                              ...
                              >
                                <td className="px-4 py-3 font-mono text-sm font-black text-foreground">
                                  {slice.symbol}
                                </td>
                                ...
                                <td className="px-4 py-3">
                                  <span
                                    className={`flex items-center gap-1 font-mono text-sm font-bold ${chg >= 0 ? "text-bullish" : "text-bearish"}`}
                                  >
                                    {chg >= 0 ? (
                                      <TrendingUp size={10} />
                                    ) : (
                    ...
                    <div className="flex items-start gap-3 rounded-xl border border-border bg-accent/20 p-4">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Quantum engine uses Monte Carlo simulation and Simulated
                        Annealing — classical algorithms, not real quantum
                        computing. Volatility is inferred from 24h price change.
                        Feed in real historical return data for production-grade
