...
function equity(p: Property) {
  return p.current_value - p.mortgage_balance;
}
function monthlyNet(p: Property) {
  return p.monthly_rent - p.monthly_expenses;
}
        ...
        <div className="space-y-4 py-2">
          ...
          <Field label="Mortgage Balance ($)">
            <input
              type="number"
              className={inputCls}
              value={f.mortgage_balance}
              onChange={set("mortgage_balance")}
              placeholder="380000"
            />
          </Field>
          <div className="grid grid-cols-2 gap-3">
...
function CapRateCalculator() {
  ...
  const rowCls =
    "flex justify-between text-sm py-1.5 border-b border-border/50 last:border-0";
    ...
    <Card className="p-5 space-y-4">
      ...
      {pp > 0 && (
        <div className="rounded-lg bg-muted/30 p-4 space-y-0.5">
          ...
          <div className={rowCls}>
            <span className="text-muted-foreground">
              Cash-on-Cash Return (20% down assumed)
            </span>
            <span
              className={
                calcCoc >= 0
                      ...
                      {properties.map((p) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <Badge
                              variant="outline"
                              className={typeBadge(p.property_type)}
                            >
                              {p.property_type}
                            </Badge>
                          </td>
                          ...
                          <td className="px-4 py-3 text-right">
                            <span
                              className={
                                equity(p) >= 0 ? "text-primary" : "text-bearish"
                              }
                            >
                              {fmtUSD(equity(p))}
                            </span>
                          ...
                          <td className="px-4 py-3 text-right">
                            {p.monthly_rent ? fmtUSD(p.monthly_rent) : "—"}
                          </td>
                          <td className="px-4 py-3 text-right text-muted-foreground">
                            {p.monthly_expenses
                              ? fmtUSD(p.monthly_expenses)
                              : "—"}
                          </td>
                          <td className="px-4 py-3 text-right">
                            <span
