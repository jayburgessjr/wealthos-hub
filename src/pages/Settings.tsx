...
import { CreditCard, ShieldCheck, Rocket, Wand2 } from "lucide-react";
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4 rounded-lg border border-border bg-card p-5">
            ...
            <button
              ...
              className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-fast hover:brightness-110 disabled:opacity-50"
            >
            ...
            {isLoading ? (
              <div className="space-y-3">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-9 animate-pulse rounded bg-accent"
                  />
                ))}
              </div>
            ) : (
            ...
            <div className="flex items-center gap-4 rounded-xl border border-border bg-background p-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground uppercase">
                  Status:{" "}
                  <span className={isPro ? "text-bullish font-bold" : ""}>
                    {subscriptionStatus}
                  </span>
                </p>
              </div>
          ...
          <div className="space-y-4 rounded-lg border border-border bg-card p-5">
            ...
            <p className="text-xs text-muted-foreground">
              Re-run the onboarding wizard to update your investor profile,
              goals, risk tolerance, and asset preferences.
            </p>
            <button
              onClick={() => navigate("/onboarding")}
              className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-fast hover:brightness-110"
            >
          ...
          <div className="space-y-4 rounded-lg border border-bearish/30 bg-card p-5">
            <h3 className="font-display text-sm font-semibold text-bearish">
              Danger Zone
            </h3>
            <p className="text-xs text-muted-foreground">
              Close all open positions and zero out portfolio stats. This cannot
              be undone.
            </p>
            <button
              onClick={() => {
                if (
                  window.confirm(
                    "Reset portfolio? This will close all positions and cannot be undone.",
                  )
                ) {
                  resetPortfolio();
                }
