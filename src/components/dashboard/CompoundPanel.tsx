...
export default function CompoundPanel() {
  ...
  const { data: settings, isLoading: settingsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (isDemoMode) return { starting_capital: 100000, monthly_contribution: 2500, reinvestment_pct: 100, risk_tier: "moderate" };
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      <div className="mb-2 flex items-baseline gap-2">
        <span className="font-mono text-xs text-muted-foreground">${startingCapital.toLocaleString()}</span>
        <span className="text-muted-foreground">→</span>
        <span className="font-mono text-lg font-bold text-bullish">${currentCapital.toLocaleString()}</span>
        <span className="text-muted-foreground">→</span>
        <span className="font-mono text-xs text-neutral">${targetCapital.toLocaleString()} target</span>
      </div>
      <ResponsiveContainer width="100%" height={140}>
        <LineChart data={projections}>
          ...
          />
          <Line type="monotone" dataKey="capital" stroke="hsl(var(--primary))" strokeWidth={2} dot={false} name="Projected" />
        </LineChart>
      </ResponsiveContainer>
      <div className="mt-3 space-y-1.5">
        {distributionList.length > 0 ? (
          ...
          <p className="text-[10px] text-muted-foreground text-center py-2 uppercase font-bold tracking-widest">No active allocations</p>
        )}
      </div>
    </div>
  );
